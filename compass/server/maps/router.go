...
func Router(r *gin.Engine) {
	...
	maps := r.Group("/api/maps")
	{
		...
		maps.GET("/cachelocations", cacheLocationProvider) // just the name and coordinate provider
		//provide locations to caches
		maps.GET("/locations/:page", locationProvider)  
		maps.GET("/location/:id", locationDetailProvider) // provide exact details about the location using the id
		maps.GET("/reviews/:id/:page", reviewProvider)  
	
		// provide the reviews of the location id, most recent 50, if there are more do the pagination
